...
-- === transactions (ledger operazioni) =======================================
...
drop policy if exists "tx: insert propri" on public.transactions;
drop policy if exists "tx: delete propri" on public.transactions;
create policy "tx: select propri" on public.transactions for select using (auth.uid() = user_id);
create policy "tx: insert propri" on public.transactions for insert with check (auth.uid() = user_id);
create policy "tx: delete propri" on public.transactions for delete using (auth.uid() = user_id);
...
-- === multi-portafoglio: strategia per-portafoglio (plans/targets/rebalance/snapshots) ===
alter table public.plans add column if not exists portfolio_id bigint references public.portfolios (id) on delete cascade;
...
alter table public.target_allocations add column if not exists portfolio_id bigint references public.portfolios (id) on delete cascade;
...
alter table public.rebalance_settings add column if not exists portfolio_id bigint references public.portfolios (id) on delete cascade;
...
alter table public.portfolio_snapshots add column if not exists portfolio_id bigint references public.portfolios (id) on delete cascade;
...
-- === modalità consulente (role + advisor_clients + RLS read-only) ===========
alter table public.profiles add column if not exists role text not null default 'retail';
...
create index if not exists advisor_clients_advisor_idx on public.advisor_clients (advisor_id);
create index if not exists advisor_clients_email_idx on public.advisor_clients (client_email);
...
create policy "advisor_clients: consulente gestisce" on public.advisor_clients for all
...
create policy "advisor_clients: cliente vede invito" on public.advisor_clients for select
  using (client_email = (auth.jwt() ->> 'email'));
drop policy if exists "advisor_clients: cliente accetta" on public.advisor_clients;
create policy "advisor_clients: cliente accetta" on public.advisor_clients for update
  using (client_email = (auth.jwt() ->> 'email')) with check (client_email = (auth.jwt() ->> 'email'));
