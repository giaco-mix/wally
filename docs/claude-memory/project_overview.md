...
- **D completata** (per-portafoglio): plans/target_allocations/rebalance_settings/portfolio_snapshots hanno `portfolio_id` + vincoli unique aggiornati; PlanRepository e relativi controller usano `currentPortfolioId`; `ensureDefaultPortfolio` fa il backfill di tutte queste tabelle. Ogni portafoglio ha la sua strategia. (demo InMemory: scoping per-portafoglio completo, seed assegnato al Principale.)
- **News arricchite**: thumbnail + tempo relativo + news di mercato generali; card "Novità" in dashboard.
- Nuove migration da applicare (tutte in `apply_all.sql`): instrument_currency_leverage, transactions, plan_pac, portfolios, tx_sleeve, **per_portfolio_strategy** (cambia vincoli unique su plans/targets/rebalance_settings/snapshots → ATTENZIONE: droppa e ricrea constraint, rieseguire apply_all è idempotente). Altre già presenti dall'altro device (web push, ecc.).
...
Sessione set 2026 (desktop, allineato dopo il lavoro del portatile): motore **rendimento per-accumulo** (S2, `lib/features/transactions/domain/lot_performance.dart` + schermata `/rendimento`, FIFO sui lotti, money-weighted); **dividendi** nel rendimento (S3, TxKind.dividend = cassa, no migration); **promemoria PAC** in-app (S4, notificationsProvider); ricerca **ISIN** via Yahoo (S1, solo hint). Vincolo: [[market-data-decision]] tutto in-house, **niente servizi terzi a pagamento**. Idee socio in `docs/feature-backlog.md` (sezione "Spunti socio"). **Pivot B2B2C consulenti**: strategia in `docs/pivot-consulenti.md` (ruoli/RLS/fasi, F0 validazione prima del codice). Onboarding collaboratore (branch+PR, Supabase reale condiviso): `docs/onboarding-collaboratore.md`.
