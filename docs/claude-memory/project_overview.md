...
- ⏳ **D multi-portafoglio rinviato di proposito** (strutturale; piano dettagliato in `docs/feature-backlog.md`). 💡 B3 (composizione PAC variabile) e G2 (app adattiva) rinviati.
- Nuove migration da applicare (incluse in `apply_all.sql`): instrument_currency_leverage, transactions, plan_pac. Altre già presenti dall'altro device (web push, ecc.).
